...
def plot(self):
    self.query_to_plot() # call the dataframes
    ...
    self.geo = px.choropleth(self.df_subset, 
                locations="iso code", 
    ...
    self.geo2 = px.scatter_geo(self.df_subset, 
                locations="iso code",
    ...
    while s < n_rows:
        ...
        for index, row in self.df_annotation.iloc[s:e].iterrows():
            self.geo.add_annotation(text=row['LastUpdate'],
                                    xref="paper", yref="paper",
                                    font = {"family": "sans-serif", 'size': 9,\
                                    "color": "MidnightBlue"},
    ...
    self.geo.add_trace(self.geo2.data[0])
    for i, j in enumerate(self.geo.frames):
        self.geo.frames[i].data += (self.geo2.frames[i].data[0],)
    # show the plot
    self.geo.show()
